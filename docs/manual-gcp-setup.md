...
_(Press Enter to accept defaults usually)_
...
1. Copy the output starting with `ssh-ed25519 ...`
2. Go to GitHub SSH Settings
3. Click **New SSH key**, give it a Title (e.g., "GCP VM"), and paste the key.
...
> If it feels like it's taking too long, or a container says unhealthy, check the logs to see what's happening.
...
Download and install the latest release. This script auto-detects your OS and installs the latest binary:

```bash
bash -c "$(curl -sL https://get.containerlab.dev)"

# permissions setup. Add yourself to the group
...
- **Tailscale Access**: If you want to access these virtual switches from your laptop via Tailscale, you have two options:
  1. **Tailscale Subnet Router**: Run `sudo tailscale up --advertise-routes=172.20.20.0/24`. This allows your laptop to "see" the internal container network.
  2. **SSH via VM**: Use your VM as a jump host.
